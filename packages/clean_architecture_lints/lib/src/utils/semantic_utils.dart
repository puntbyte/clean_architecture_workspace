...
/*class SemanticUtils {
  ...
  /// interface.
  static bool isArchitecturalOverride(ExecutableElement element, ComponentResolver resolver) {
    ...
    // Construct the unique `Name` identifier for the member.
    final memberName = Name(library.uri, elementName);

    // Check all supertypes (from extends, implements, with).
    for (final supertype in enclosingClass.allSupertypes) {
      final superElement = supertype.element;
      final source = superElement.firstFragment.libraryFragment.source;
      final superComponent = resolver.resolveComponent(source.fullName);

      if (superComponent == null) continue;

      // An architectural contract is a repository_contract or source_contract.
      // This can be expanded in the future.
      final isContract = superComponent.kind == ComponentKind.repositoryContract ||
          superComponent.kind == ComponentKind.sourceContract;

      if (isContract) {
        // Does this contract interface declare a member with the same name?
        if (superElement.lookUpInheritedMethod(elementName, library: library) != null ||
            superElement.lookUpInheritedGetter(elementName, library: library) != null) {
          return true; // Found an override from a valid contract.
        }
      }
    }
