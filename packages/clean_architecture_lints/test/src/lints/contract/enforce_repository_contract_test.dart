...
void main() {
  group('EnforceRepositoryContract Lint', () {
    ...
    setUp(() {
      ...
      writeFile(p.join(projectPath, 'lib', 'core', 'repository', 'repository.dart'),
    ...
    test('should report violation when repository contract does not extend Repository', () async {
      final path = p.join(projectPath, 'lib', 'features', 'user', 'domain', 'contracts', 'user_repository.dart');
    ...
    test('should not report violation when repository contract correctly extends Repository', () async {
      final path = p.join(projectPath, 'lib', 'features', 'order', 'domain', 'contracts', 'order_repository.dart');
      writeFile(path, '''
    ...
    test('should not report violation for a concrete class in the contract layer', () async {
      final path = p.join(projectPath, 'lib', 'features', 'shared', 'domain', 'contracts', 'concrete_repo.dart');
    ...
    test('should be ignored when a custom inheritance rule for contracts is defined', () async {
      final path = p.join(projectPath, 'lib', 'features', 'custom', 'domain', 'contracts', 'custom_repo.dart');
      writeFile(path, 'abstract class CustomRepo {}');

      final lints = await runLint(filePath: path, inheritances: [{'on': 'contract'}]);
